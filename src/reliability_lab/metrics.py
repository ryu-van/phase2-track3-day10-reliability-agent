...
class RunMetrics(BaseModel):
    ...
    def write_csv(self, path: str | Path) -> None:
        ...
        for key, value in report.items():
            if key == "scenarios":
                # Flatten scenarios: each scenario becomes "scenario_{name}"
                for scenario_name, scenario_result in value.items():
                    flat_report[f"scenario_{scenario_name}"] = scenario_result
            else:
